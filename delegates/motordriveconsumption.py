...
class MotorDriveConsumption(SystemCalcDelegate):
    ...
    consumption_window = [[0, 0] for _ in range(WINDOW_SEGMENT_COUNT)]
    consumption_window_index = 0
    consumption_window_total_distance = 0.0
    consumption_window_total_energy = 0.0
    ...
    def get_output(self):
        return [
            (PREFIX + "/Consumption", {"gettext": "%dkWh/km"}),
        ]
    ...
    def service_registered(self):
        self._dbusmonitor.track_value(
            self._dbusservice.name, "/GpsService", self._on_gpsservice_changed
        )
    ...
    def _calculate_distance_traveled(self, point, dt):
        ...
        if self.last_point is not None:
            ...
            self.is_continuous = distance_raw <= distance_max
    ...
    def update_values(self, newvalues):
        if self.consumption_window_total_distance > 0:
            consumption = (self.consumption_window_total_energy / 1000.0) / (
